 ...
 * * @spec: specifier * @f: The function associated 
 * */
typedef struct spec_types{	char *spec;	
char *(*f)();} s_types;
char *(*get_spec_func(char s))(va_list);int _printf(const char *format, ...);
...
char *src, unsigned int n, unsigned int bufferlen);
